# -*- coding: utf-8 -*-
"""数学练习题通用渲染器：读取 <年级>数学/data_up.py 与 data_down.py，生成10份HTML练习卷
用法: python gen_any.py 四年级    (目录名 = 年级 + "数学")"""
import os, sys
...
<title>__GRADE__数学 __TITLE__</title>
<style>
...
<div class="wrap">
<header class="paper">
  <div class="sub">人教版小学数学__GRADE__ · 专项练习</div>
  <h1>__TITLE__</h1>
...
<section class="block notice">
  ...
  <p class="hint">评分参考：填空每空1分；判断、选择每题2分；口算每题1分；竖式每题4分（验算占1分）；解决问题每题6分（列式3分＋计算2分＋单位答句1分）。错题请回到《__GRADE__数学知识点与考点总览》对应单元查漏补缺。</p>
</section>
__BODY__
<div class="answer">
<details class="ans">
...
<footer>人教版__GRADE__数学专项练习 · 闫胜君整理 · 建议先限时完成再核对答案，错题对照《__GRADE__数学知识点与考点总览》标注薄弱单元</footer>
</div>
</body>
</html>"""
...
def build(ex, outdir, grade):
    ...
    html = (TPL.replace("__TITLE__", ex["title"])
               ...
               .replace("__TIME__", ex.get("time", "40分钟"))
...
def main():
    grade = sys.argv[1] if len(sys.argv) > 1 else "四年级"
    base = os.path.dirname(os.path.abspath(__file__))
    outdir = os.path.join(base, grade + "数学")
    sys.path.insert(0, outdir)
    from data_up import EXERCISES_UP
    from data_down import EXERCISES_DOWN
    total = 0
    for ex in EXERCISES_UP + EXERCISES_DOWN:
        total += build(ex, outdir, grade)
    print(f"\n{grade}共生成 {len(EXERCISES_UP) + len(EXERCISES_DOWN)} 份练习，合计 {total} 题")
