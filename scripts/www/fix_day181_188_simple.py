...
def generate_choiceQuestions_js(day_num):
    ...
    for i, q in enumerate(questions):
        ...
        for j, opt in enumerate(q['options']):
            ...
            # 转义文本中的单引号
            text_escaped = opt['text'].replace('\\', '\\\\').replace("'", "\\'")
            js_lines.append(f'                    {{ text: \'{text_escaped}\', correct: {correct_str} }},')
        js_lines.append('                ],')
...
def fix_file(filepath):
    ...
    if choiceQuestions_js:
        ...
        if content_new == content:
            print(f'  [WARN] Cannot replace choiceQuestions: {filename}')
            return False
        content = content_new
        print(f'  [OK] Fixed choiceQuestions: {filename}')
    ...
    if re.search(old_btn_pattern, content):
        ...
        print(f'  [OK] Fixed essay button: {filename}')
    else:
        print(f'  [INFO] No reference-btn found in: {filename}')
    ...
    if 'class="reference-btn"' in content:
        print(f'  [WARN] Still has reference-btn: {filename}')
    
    # 3. 检查footer是否存在
    if 'footer-support' not in content:
        print(f'  [WARN] Missing footer: {filename}')
...
def main():
    ...
    print('Starting batch fix for day181-day188...')
    print('=' * 50)
    ...
    print('\n' + '=' * 50)
    print(f'Fix completed: {success_count}/{len(files_to_fix)} files successful')
